...
class FakeQueryStatsCursor:
    """Minimal fake cursor for get_paper_counts_by_query_field's GROUP BY query."""

    def __init__(self, rows: list[tuple]):
        self._rows = rows  # (field_value, paper_id)
        self._result = None

    def execute(self, sql, params=()):
        assert "search_queries" in sql and "paper_queries" in sql
        counts: dict[str, set] = {}
        for value, paper_id in self._rows:
            if value is None:
                continue
            counts.setdefault(value, set()).add(paper_id)
        self._result = sorted(
            ((value, len(paper_ids)) for value, paper_ids in counts.items()),
...
def test_paper_counts_by_query_field() -> None:
    cursor = FakeQueryStatsCursor([
        ("Green roof", 1), ("Green roof", 2), ("Rainwater harvesting", 1), (None, 3),
    ])
    assert main.get_paper_counts_by_query_field(cursor, "design_strategy") == [
        {"name": "Green roof", "paper_count": 2},
        {"name": "Rainwater harvesting", "paper_count": 1},
    ]
    print("backend get_paper_counts_by_query_field self-check passed")


def test_ecosystem_service_paper_counts_map_reduces_through_ds_es_graph() -> None:
    ds_counts = [
        {"name": "Green roof", "paper_count": 10},
        {"name": "Rainwater harvesting", "paper_count": 5},
        {"name": "Unmapped strategy", "paper_count": 99},  # not in the DS->ES graph, contributes nowhere
    ]
    # "Green roof" generates two services, so its papers count toward both;
    # "Water storage" is generated by two DS, so it sums across them.
    ds_es_pairs = [
        ("Green roof", "Evaporative cooling"),
        ("Green roof", "Water storage"),
        ("Rainwater harvesting", "Water storage"),
        ("Design strategy with no matched papers", "Habitat"),  # not in ds_counts, contributes nothing
    ]
    with patch.object(main, "get_paper_counts_by_query_field", return_value=ds_counts), \
         patch.object(main, "get_design_strategy_ecosystem_service_pairs", return_value=ds_es_pairs):
        result = main.get_ecosystem_service_paper_counts(cursor=None)

    assert result == [
        {"name": "Water storage", "paper_count": 15},
        {"name": "Evaporative cooling", "paper_count": 10},
    ]
    print("backend get_ecosystem_service_paper_counts self-check passed")


class FakeYearStatsCursor:
    """Minimal fake cursor for get_paper_year_counts's GROUP BY query."""

    def __init__(self, rows: list[tuple]):
        self._rows = rows  # (design_strategy, year, paper_id)
        self._result = None

    def execute(self, sql, params=()):
        assert "search_queries" in sql and "papers" in sql
        (design_strategies,) = params
        counts: dict[int, set] = {}
        for ds, year, paper_id in self._rows:
            if ds not in design_strategies or year is None:
                continue
            counts.setdefault(year, set()).add(paper_id)
        self._result = sorted(
            ((year, len(paper_ids)) for year, paper_ids in counts.items()),
        )
...
def test_paper_year_counts_by_design_strategy() -> None:
    cursor = FakeYearStatsCursor([
        ("Green roof", 2020, 1), ("Green roof", 2020, 2), ("Green roof", 2021, 3),
        ("Rainwater harvesting", 2020, 4), ("Green roof", None, 5),
    ])
    assert main.get_paper_year_counts(cursor, "design_strategy", "Green roof") == [
        {"year": 2020, "paper_count": 2},
        {"year": 2021, "paper_count": 1},
    ]
    print("backend get_paper_year_counts design_strategy self-check passed")


def test_paper_year_counts_by_ecosystem_service_maps_through_ds_es_graph() -> None:
    cursor = FakeYearStatsCursor([
        ("Green roof", 2020, 1), ("Rainwater harvesting", 2020, 2), ("Rainwater harvesting", 2021, 3),
    ])
    ds_es_pairs = [("Green roof", "Water storage"), ("Rainwater harvesting", "Water storage")]
    with patch.object(main, "get_design_strategy_ecosystem_service_pairs", return_value=ds_es_pairs):
        result = main.get_paper_year_counts(cursor, "ecosystem_service", "Water storage")
    assert result == [{"year": 2020, "paper_count": 2}, {"year": 2021, "paper_count": 1}]

    with patch.object(main, "get_design_strategy_ecosystem_service_pairs", return_value=ds_es_pairs):
        assert main.get_paper_year_counts(cursor, "ecosystem_service", "Unmapped service") == []
    print("backend get_paper_year_counts ecosystem_service self-check passed")
...
if __name__ == "__main__":
    main_()
    test_paper_counts_by_query_field()
    test_ecosystem_service_paper_counts_map_reduces_through_ds_es_graph()
    test_paper_year_counts_by_design_strategy()
    test_paper_year_counts_by_ecosystem_service_maps_through_ds_es_graph()
